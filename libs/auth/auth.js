...
passport.use(new BearerStrategy(
    function (accessToken, done) {
        AccessToken.findOne({ token: accessToken }, function (err, token) {
            ...
            if (Math.round((Date.now() - token.created) / 1000) > process.env.tokenLife) {
                //chenged remove to deleteone, check for any error remove comment
                //AccessToken.remove({ token: accessToken }, function (err) {
                AccessToken.deleteOne({ token: accessToken }, function (err) {
                    if (err) {
                        return done(err);
                    }
                });
